...
def chunk_name(chunk_key) -> int:
    global data
    chunk = data["_chunks"][chunk_key]
    if "name" in chunk[VARS]:
        return chunk[VARS]["name"][VALUE]
    else:
        return f"#{chunk_id(chunk_key)}"
...
def Create_IsSceneBlockActive(func_key, scene_mod_key, on_false_CutControl_key, on_true_CutControl_key, graph_key,
                              graph_path) -> dict:
    return {
        "_type": "CQuestScriptBlock",
        "_key": func_key,
...
def Create_ConditionBlock(cond_key, graph_key, putName, scene_vanilla_key, scene_mod_key, script_vanilla_key,
                          script_mod_key) -> dict:
    return {
...
def handle_scene_chunk(scene_key, graph_key, quest_path):
    ...
    data["_chunks"][vanilla_func_key] = Create_SetSceneBlockActive(vanilla_func_key, scene_copy_key, graph_key,
                                                                   quest_path, False)
    ...
    data["_chunks"][mod_func_key] = Create_SetSceneBlockActive(mod_func_key, scene_copy_key, graph_key, quest_path,
                                                               True)
    ...
    for thunder in graph_data[scene_key]["thunder"]:
        ...
        if thunder["key"] in mod_key_by_key.values():
            ...
            if thunder["key"] not in mod_key_by_key:
                ...
                data["_chunks"][script_check_key] = Create_IsSceneBlockActive(script_check_key, scene_copy_key,
                                                                              thunder["key"], thunder_mod_key,
                                                                              graph_key, quest_path)
        ...
        for socket in data["_chunks"][thunder_mod_key]["_vars"]["cachedConnections"]["_elements"]:
            if "socketId" in socket["_vars"] and "blocks" in socket["_vars"] and socket["_vars"]["socketId"][
                "_value"] == "Thunder":
...
def main():
    ...
    with open("w2scene_replacements_mods.csv", encoding="utf-8", mode="r") as infile:
        for line in infile.readlines():
            line = line[:-1]
            if line.startswith(";"):
                continue
    ...
    dir = input("Input dir (Quests): ")
    # edited_copy_path = input("Output copy dir (UncookedQuests.Vanilla): ")
    edited_dir = input("Output dir (Quests.Patched): ")
    ...
    for w2quest_path in tqdm(w2quest_paths):
        ...
        for key in data["_chunks"]:
            if key.startswith("CQuestChangeWorldBlock"):
                WC_blocks.append(key)

        for key in WC_blocks:
            info(f"WORLD: {key} ({w2quest_path})")
            parent_key = data["_chunks"][key]["_parentKey"]
            pause_key = f"CQuestPauseConditionBlock #{chunk_count()}"
            cond_key = f"NR_QuestCond_PlayerManagerReady #{chunk_count() + 1}"

            replace_key = key
            while True:
                if not "cachedConnections" in data["_chunks"][replace_key][VARS] or len(data["_chunks"][replace_key][VARS]["cachedConnections"][ELEMENTS]) > 1:
                    break
                conn_vars = data["_chunks"][replace_key][VARS]["cachedConnections"][ELEMENTS][0][VARS]
                if "blocks" not in conn_vars or len(conn_vars["blocks"][ELEMENTS]) > 1:
                    break
                if "ock" not in conn_vars["blocks"][ELEMENTS][0][VARS]:
                    break
                next_key = conn_vars["blocks"][ELEMENTS][0][VARS]["ock"][VARS][REFERENCE][VALUE]
                if not next_key.startswith("CQuestPauseConditionBlock"):
                    break
                replace_key = next_key

            print(f"Replace key: {replace_key}")
            replace_conn_vars = data["_chunks"][replace_key][VARS]["cachedConnections"][ELEMENTS][0][VARS]
            if "blocks" not in replace_conn_vars:
                replace_blocks = []
            else:
                replace_blocks = deepcopy(replace_conn_vars["blocks"][ELEMENTS])
                replace_conn_vars["blocks"][ELEMENTS] = [
                    {
                        "_type": "SBlockDesc",
                        "_vars": {
                            "ock": {
            ...
            data["_chunks"][pause_key] = {
                "_type": "CQuestPauseConditionBlock",
                "_key": pause_key,
                ...
                "_vars": {
                    "name": {
                        "_type": "String",
                        "_value": f"[Ensure NR Player loaded after {chunk_name(key)}]"
                    },
                    "guid": {
                        "_type": "CGUID",
                        "_value": new_guid()
                    ...
                    "conditions": {
                        "_type": "array:2,0,ptr:IQuestCondition",
                        "_elements": [
                            {
                                "_type": "ptr:IQuestCondition",
                                "_vars": {
            ...
            Add_GraphBlock(pause_key, parent_key)
            data["_chunks"][cond_key] = {
                "_type": "NR_QuestCond_PlayerManagerReady",
                "_key": cond_key,
                "_parentKey": pause_key,
                "_flags": 0,
                "_vars": {
                    "_unknownBytes": {
                        "_type": "CBytes",
        ...
        if WC_blocks:
            ...
            # edited_copy_path = edited_copy_dir + "/" + str(w2quest_path.relative_to(dir))
            print(f"Save edited file: {edited_path}")
            os.makedirs(os.path.dirname(edited_path), exist_ok=True)
            # os.makedirs(os.path.dirname(edited_copy_path), exist_ok=True)
