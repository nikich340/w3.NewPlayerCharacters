...
statemachine class NR_MagicManager extends IScriptable {
	...
	protected var mPolymorphismAnimalType : name;
	...
	public function PreviewColor(actionType : ENR_MagicAction, sign : ESignType, optional horse : bool) : ENR_MagicColor {
		var prefix : String;
		...
		return (ENR_MagicColor)sMap[sign].getI(prefix + ENR_MAToName(actionType), ENR_ColorWhite);
	}
	...
	public function PreviewLocActionColored(actionType : ENR_MagicAction, optional horse : bool, optional underwater : bool) : String {
		...
		if (!IsActionLearned(finalType) && !IsActionLearning(finalType)) {
			return NR_ColorFormattedText( ENR_MAToLocString(finalType) + " [" + GetLocStringById(1066070) + "]", ENR_ColorRed );
		}
		return NR_ColorFormattedText( ENR_MAToLocString(finalType), PreviewColor(finalType, eqSign, horse) );
	}
	...
	protected function ColorFormattedValue(valueId : int, color : ENR_MagicColor) : String {
		return NR_ColorFormattedText( GetLocStringById(valueId), color );
	}
	...
	public function ShowMagicInfo(sectionName : name) {
		...
		if (sectionName == 'main') {
			...
			for (i = 0; i < signsAlphabetically.Size(); i += 1) {
				...
				text += NR_GetSignIconFormattedByType(s,,,-20) + "{ }" + NR_ColorFormattedText(styeText, color);
				text += BR;
			}
			...
			for (i = 0; i < signsAlphabetically.Size(); i += 1) {
				...
				text += NR_GetSignIconFormattedByType(s,,,-20) + "{ }" + "{2115940148}:{ }" + ColorFormattedValue(styleId, color) + "/" + NR_ColorFormattedText("*", color2) + ";{ }";
				...
				text += "{2115940149}:{ }" + ColorFormattedValue(NR_ColorLocId(color), color) + "";
				text += BR;
			}
            ...
            for (i = 0; i < signsAlphabetically.Size(); i += 1) {
                ...
                if (typeId == ENR_SpecialControl) {
                    ...
                    text += "{2115940156}:{ }" + ColorFormattedValue(NR_ColorLocId(color), color);
            ...
            for (i = 0; i < signsAlphabetically.Size(); i += 1) {
                ...
                if (typeId == ENR_SpecialLightningFall) {
                    ...
                    text += "{2115940163}:{ }" + ColorFormattedValue(NR_ColorLocId(color), color);
	...
	public function SetPolymorphismAnimalType(animalType : name) {
		mPolymorphismAnimalType = animalType;
	}

	public function GetPolymorphismAnimalType() : name {
		return mPolymorphismAnimalType;
	}
	...
	function SetDefaults_LightThrow() {
		...
		// horse attacks
		sMap[ST_Axii].setI("color_horse_" + ENR_MAToName(ENR_Lightning), ENR_ColorYellow);
		sMap[ST_Axii].setI("color_horse_" + ENR_MAToName(ENR_ProjectileWithPrepare), ENR_ColorRed);
	...
	function SetColorPerSignActions(sign : ESignType, color : ENR_MagicColor) {
		if ( IsActionCustomizationUnlocked(ENR_HandFx) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_HandFx), color);

		if ( IsActionCustomizationUnlocked(ENR_Teleport) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_Teleport), color);
		
		if ( IsActionCustomizationUnlocked(ENR_FastTravelTeleport) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_FastTravelTeleport), color);
		
		if ( IsActionCustomizationUnlocked(ENR_Slash) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_Slash), color);

		if ( IsActionCustomizationUnlocked(ENR_ThrowAbstract) ) {
			sMap[sign].setI("color_" + ENR_MAToName(ENR_ThrowAbstract), color);
			sMap[sign].setI("color_horse_" + ENR_MAToName(ENR_Lightning), color);
			sMap[sign].setI("color_horse_" + ENR_MAToName(ENR_ProjectileWithPrepare), color);
		}

		if ( IsActionCustomizationUnlocked(ENR_Rock) ) {
			sMap[sign].setI("color_" + ENR_MAToName(ENR_Rock), color);
			sMap[sign].setI("color_cone_" + ENR_MAToName(ENR_Rock), color);
		}
		
		if ( IsActionCustomizationUnlocked(ENR_BombExplosion) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_BombExplosion), color);
		
		if ( IsActionCustomizationUnlocked(ENR_CounterPush) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_CounterPush), color);

		if ( IsActionCustomizationUnlocked(ENR_SpecialMeteor) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialMeteor), color);
		
		if ( IsActionCustomizationUnlocked(ENR_SpecialShield) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialShield), color);

		if ( IsActionCustomizationUnlocked(ENR_SpecialServant) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialServant), color);

		if ( IsActionCustomizationUnlocked(ENR_SpecialLightningFall) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialLightningFall), color);
		if ( IsActionCustomizationUnlocked(ENR_SpecialField) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialField), color);
		if ( IsActionCustomizationUnlocked(ENR_SpecialMeteorFall) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialMeteorFall), color);
		if ( IsActionCustomizationUnlocked(ENR_SpecialLumos) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialLumos), color);
		if ( IsActionCustomizationUnlocked(ENR_SpecialPolymorphism) )
			sMap[sign].setI("color_" + ENR_MAToName(ENR_SpecialPolymorphism), color);
	}
	...
	public function ForceStopPolymorphism(reason : String) {
		var i : int;

		for (i = 0; i < cachedActions.Size(); i += 1) {
			if (cachedActions[i].actionType == ENR_SpecialPolymorphism) {
				NR_Info("NR_MagicManager.ForceStopPolymorphism: reason = " + reason);
				((NR_MagicSpecialPolymorphism)cachedActions[i]).ForceStop();
				break;
			}
		}
	}
	...
	/* DAMAGE & SKILLS */
	public function GetSkillLevel() : ENR_MagicSkillLevel
	{
	...
	public function GetPossibleSkillLevel() : ENR_MagicSkillLevel
	{
		...
		for ( skillLevel = ENR_SkillArchMistress; skillLevel >= ENR_SkillNovice; skillLevel -= 1 ) {
			if ( playerLevel >= GetPlayerLevelForSkillLevel((ENR_MagicSkillLevel)skillLevel) ) {
				return (ENR_MagicSkillLevel)skillLevel;
			}
		}
		return ENR_SkillNovice;
	}

	public function GetPlayerLevelForSkillLevel(skillLevel : ENR_MagicSkillLevel) : int {
		...
		switch (skillLevel) {
			...
			case ENR_SkillExperienced:
				return FloorF(startLevel + (playerMax - startLevel) * 26 / 100);
			case ENR_SkillMistress:
				return FloorF(startLevel + (playerMax - startLevel) * 46 / 100);
			case ENR_SkillArchMistress:
	...
	public function GetSkillLevelLocStr(skillLevel : ENR_MagicSkillLevel) : String
	{
		//var skillLevel : ENR_MagicSkillLevel = GetSkillLevel();
		switch (skillLevel) {
			case ENR_SkillApprentice:
...
state MagicLoop in NR_MagicManager {
	...
	latent function PerformSwimmingAction() {
		...
		if (!target || !target.IsSwimming() || target.HasTag('NR_MagicWaterTrap'))
			return;
