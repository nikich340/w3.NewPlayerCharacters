...
exec function NRRandomColored(text : String) {
	NR_Notify(NR_ColorFormattedText(text, ENR_ColorRandom));
}
...
exec function NRMagicLevels() {
	var str : String;

	str += "ENR_SkillNovice: " + NR_GetMagicManager().GetPlayerLevelForSkillLevel(ENR_SkillNovice);
	str += "<br>ENR_SkillApprentice: " + NR_GetMagicManager().GetPlayerLevelForSkillLevel(ENR_SkillApprentice);
	str += "<br>ENR_SkillExperienced: " + NR_GetMagicManager().GetPlayerLevelForSkillLevel(ENR_SkillExperienced);
	str += "<br>ENR_SkillMistress: " + NR_GetMagicManager().GetPlayerLevelForSkillLevel(ENR_SkillMistress);
	str += "<br>ENR_SkillArchMistress: " + NR_GetMagicManager().GetPlayerLevelForSkillLevel(ENR_SkillArchMistress);
	NR_Notify(str);
}
...
exec function nrSkill(skillLevel : int) {
	if (skillLevel > EnumGetMax('ENR_MagicSkillLevel') || skillLevel < 1) {
		NR_Notify("Invalid skill value, it must be [1; " + EnumGetMax('ENR_MagicSkillLevel') + "]");
		return;
	}
...
exec function nrshowlocks() {
	var locks : array< array<SInputActionLock> >;
	var i, j : int;
...
exec function gptest() {
	var scene      : CStoryScene;
	scene = (CStoryScene)LoadResource("quests\minor_quests\prologue_village\quest_files\mq0001_missing_brother\scenes\mq0001_03_02_gp_geralt.w2scene", true);
	if (!scene)
		NR_Error("NULL scene!");
...
exec function setfact(factID : string, value: int)
{
	FactsSet(factID, value);
}
...
//pspawn(dlc\dlcchristmastreeprivate\data\entities\vegetation\christmas_tree.w2ent, 0)
