...
// resets all current appearance and sets up completely new
// CORE function for external mods
...
// Only when sorceress type: checks if player reached sorceress skill level
/* API */ quest function NR_HasPlayerReachedMagicLevel_Q(requiredLevel : ENR_MagicSkillLevel) : bool {
    var nr_manager : NR_MagicManager = NR_GetMagicManager();

    if (nr_manager)
        return nr_manager.GetSkillLevel() >= requiredLevel;
    else
        return false;
}

// Only when sorceress type: checks if player has learned specific magic action
/* API */ quest function NR_HasPlayerLearnedMagicAction_Q(magicAction : ENR_MagicAction, treatLearningAsLearned : bool) : bool {
    var nr_manager : NR_MagicManager = NR_GetMagicManager();

    if (nr_manager) {
        if (treatLearningAsLearned)
            return nr_manager.IsActionLearned(magicAction) || nr_manager.IsActionLearning(magicAction);
        else
            return nr_manager.IsActionLearned(magicAction);
    } else {
        return false;
    }
}

// Only when sorceress type: checks if player has some ability unlocked for specified action
/* API */ quest function NR_HasPlayerUnlockedMagicActionAbility_Q(magicAction : ENR_MagicAction, abilityName : String) : bool {
    var nr_manager : NR_MagicManager = NR_GetMagicManager();

    if (nr_manager)
        return nr_manager.IsActionAbilityUnlocked(magicAction, abilityName);
    else
        return false;
}

quest function NR_SetLumosFx_Q(enable : bool, optional fxName : name) {
    NR_GetMagicManager().LumosFX(enable, fxName);
}
